...
contract Price_fixing {
    ...
    modifier ongoingContractStateOnly() {
        if (contractState != State.Ongoing) {
            revert intermittentContractState(
                "Currently the contract is not accepting any feeds!"
            );
        }
        _;
    }
    ...
    function terminateConsensusBasedPriceFixingSession()
        public
        ongoingContractStateOnly
    {
        contractState = State.Intermittent;
    }

    function castPriceView(
        uint _price
    ) public registeredDriversOnly ongoingContractStateOnly {
        if (_price < 0) {
            revert negativePrice("Please enter a positive price value!");
        }
        if (pricesCasted[msg.sender] > 0) {
            revert alreadyCastedPrice(
                "Please refer to the updateCast() function to update your feed."
            );
        }
    ...
    function updateCast(
        ...
        if (pricesCasted[msg.sender] == 0) {
            revert didNotCastBefore(
                "Please refer to the castPriceView() function to first cast a price view before manipulating it."
            );
        }
    ...
    function calculateMedian() public {
        ...
        if (count == 0) {
            revert noFeedsCastedYet(
                "No feeds casted yet! Cannot calculate median!"
            );
        }
        ...
        for (uint i = 0; i < count; i++) {
            ...
            if (pricesCasted[addr] > 0) {
                // check required since a driver might delete his account after casting his feed and that operation would be reflected in the mapping
